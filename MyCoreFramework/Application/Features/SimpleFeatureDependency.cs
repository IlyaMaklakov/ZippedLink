    ...
    public class SimpleFeatureDependency : IFeatureDependency
    {
        ...
        public Task<bool> IsSatisfiedAsync(IFeatureDependencyContext context)
        {
            return context.TenantId.HasValue
                ? context.FeatureChecker.IsEnabledAsync(context.TenantId.Value, this.RequiresAll, this.Features)
                : context.FeatureChecker.IsEnabledAsync(this.RequiresAll, this.Features);
        }
    }
}