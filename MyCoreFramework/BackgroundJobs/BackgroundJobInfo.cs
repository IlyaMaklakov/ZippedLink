...
namespace MyCoreFramework.BackgroundJobs
{
    ...
    public class BackgroundJobInfo : CreationAuditedEntity<long>
    {
        ...
        {
            this.NextTryTime = Clock.Now;
            this.Priority = BackgroundJobPriority.Normal;
        }
        ...
        {
            var nextWaitDuration = DefaultFirstWaitDuration * (Math.Pow(DefaultWaitFactor, this.TryCount - 1));
            var nextTryDate = this.LastTryTime.HasValue
                ? this.LastTryTime.Value.AddSeconds(nextWaitDuration)
                : Clock.Now.AddSeconds(nextWaitDuration);

            if (nextTryDate.Subtract(this.CreationTime).TotalSeconds > DefaultTimeout)
            {
                return null;
            }
