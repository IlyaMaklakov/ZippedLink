...
namespace MyCoreFramework.Auditing
{
    ...
    public class AuditInfo
    {
        ...
        {
            var loggedUserId = this.UserId.HasValue
                                   ? "user " + this.UserId.Value
                                   : "an anonymous user";

            var exceptionOrSuccessMessage = this.Exception != null
                ? "exception: " + this.Exception.Message
                : "succeed";

            return $"AUDIT LOG: {this.ServiceName}.{this.MethodName} is executed by {loggedUserId} in {this.ExecutionDuration} ms from {this.ClientIpAddress} IP address with {exceptionOrSuccessMessage}.";
        }
    }
}