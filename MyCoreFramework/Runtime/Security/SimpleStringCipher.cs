...
namespace MyCoreFramework.Runtime.Security
{
    /// <summary>
    ...
    public class SimpleStringCipher
    {
        ...
        public string Encrypt(string plainText, string passPhrase = DefaultPassPhrase)
        {
            ...
            using (PasswordDeriveBytes password = new PasswordDeriveBytes(passPhrase, null))
            {
                ...
                using (RijndaelManaged symmetricKey = new RijndaelManaged())
                {
                    ...
                    using (ICryptoTransform encryptor = symmetricKey.CreateEncryptor(keyBytes, this.InitVectorBytes))
                    {
        ...
        public string Decrypt(string cipherText, string passPhrase = DefaultPassPhrase)
        {
            ...
            using (PasswordDeriveBytes password = new PasswordDeriveBytes(passPhrase, null))
            {
                ...
                {
                    symmetricKey.Mode = CipherMode.CBC;
                    using (ICryptoTransform decryptor = symmetricKey.CreateDecryptor(keyBytes, this.InitVectorBytes))
                    {
